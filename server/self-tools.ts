...
export function createSelfMcp() {
  return createSdkMcpServer({
    ...
    tools: [
      tool(
        "get_config",
        "Return Boop's runtime configuration: which AI provider/model it's using, which integrations are loaded, and basic env info. Use when the user asks 'what model are you?', 'what version?', or anything about the agent itself.",
        {},
        async () => {
          ...
          const config = {
            ...
            envDefault: defaultModel(),
            availableModels: modelsForCurrentProvider(),
...
Use when the user says "use opus", "switch to sonnet", "use codex", "make it faster (haiku)", etc.

Cost note for Claude API-key mode (approximate, per 1M output tokens): Opus 4.7 ≈ $75, Sonnet 4.6 ≈ $15, Haiku 4.5 ≈ $4. Mention briefly when switching to Opus.`,
        {
          model: z
            .string()
            .describe('Model to use. Canonical ID like "claude-opus-4-7" / "gpt-5.3-codex" or alias like "opus" / "codex".'),
        },
        async ({ model }) => {
          ...
          if (!modelMatchesProvider(resolved)) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: `Model "${resolved}" is not valid for AI_PROVIDER=${aiProvider()}. Try one of: ${modelsForCurrentProvider().join(", ")}.`,
                },
              ],
            };
          }
