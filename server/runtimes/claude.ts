...
import { aggregateUsageFromResult } from "../usage.js";
...
export async function runClaudeAgent(request: RuntimeRunRequest): Promise<RuntimeRunResult> {
  const servers = new Map<string, RuntimeTool[]>();
  for (const runtimeTool of request.tools) {
    const list = servers.get(runtimeTool.namespace) ?? [];
    list.push(runtimeTool);
    servers.set(runtimeTool.namespace, list);
  }

  const mcpServers = {
    ...(request.claudeMcpServers ?? {}),
    ...Object.fromEntries(
      [...servers.entries()].map(([name, tools]) => [name, createClaudeMcpServer(name, tools)]),
    ),
  } as Record<string, McpServerConfig>;

  let text = "";
  let lastAssistantText = "";
  let usage = { ...EMPTY_RUNTIME_USAGE };
